...
//				System.out.println("宽   "+size.width+"   高"+size.height);
						...
						{	
							...
							if (lenght == 1)
							{
								...
								if (null != fieldname)
								{
									...
									closeCamera();
									Intent intent = new Intent();//TODO:解析结果
									number = fieldvalue[0];
									color = fieldvalue[1];
	...
	*/
	...
	public String savePicture(Bitmap bitmap)
	{
		...
		currentPicFile = new File(strCaptureFilePath);
		...
		try
		{
			currentPicFile.createNewFile();
			BufferedOutputStream bos = new BufferedOutputStream(new FileOutputStream(currentPicFile));
