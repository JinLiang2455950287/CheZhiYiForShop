package com.ruanyun.chezhiyi.commonlib.hxchat;
...
import com.ruanyun.chezhiyi.commonlib.hxchat.db.HXDBManager;
import com.ruanyun.chezhiyi.commonlib.hxchat.db.InviteMessgeDao;
import com.ruanyun.chezhiyi.commonlib.hxchat.db.UserDao;
...
import com.ruanyun.chezhiyi.commonlib.hxchat.parse.UserProfileManager;
import com.ruanyun.chezhiyi.commonlib.hxchat.receiver.CallReceiver;
import com.ruanyun.chezhiyi.commonlib.hxchat.ui.ChatActivity;
import com.ruanyun.chezhiyi.commonlib.hxchat.ui.VideoCallActivity;
import com.ruanyun.chezhiyi.commonlib.hxchat.ui.VoiceCallActivity;
import com.ruanyun.chezhiyi.commonlib.hxchat.utils.PreferenceManager;
...
public class HXHelper {
    ...
    public boolean isChatPageShow = false;//判断聊天页面是否在前台;
    /**
     * EMEventListener
     */
    ...
    private HxUser hxuser = new HxUser();
    ...
    protected void setEaseUIProviders() {
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotificationInfoProvider() {
            ...
            public Intent getLaunchIntent(EMMessage message) {
                ...
                } else if (!TextUtils.isEmpty(message.getStringAttribute("remindType", ""))) {//后台推送消息
                    intent = new Intent(appContext, SystemRemindActivity.class);
...
//        Intent intent = new Intent(appContext, MainActivity.class);
        ...
        if (App.getInstance().isClient()) {
            intent.setClassName(appContext, "com.ruanyun.czy.client.MainActivity");
        } else {
            intent.setClassName(appContext, "com.ruanyun.chezhiyi.MainActivity");
        }
...
//        Intent intent = new Intent(appContext, MainActivity.class);
    ...
    private EaseUser getUserInfo(String username) {
       ...
       /* if(username.equals(EMClient.getInstance().getCurrentUser()))
        ...
        if (username.equals(EMClient.getInstance().getCurrentUser()))
            return getUserProfileManager().getCurrentUserInfo();
        HxUser hxUser = DbHelper.getInstance().getUserByNum(username);

        if (hxUser != null) {
            user = new EaseUser(username);
            user.setAvatar(hxUser.getUserPhoto());
            user.setNick(hxUser.getUserNick());
            EaseCommonUtils.setUserInitialLetter(user);
        } else {
            getChatUserInfoById(username);
        }
...
//        }
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    if (!TextUtils.isEmpty(message.getStringAttribute("remindType", ""))) {        //后台推送消息
                        ...
                        if (message.getStringAttribute("remindType", "").equals(RemindInfo.REMIND_TYPE_WORKORDER/*"GD"*/) && !App.getInstance().isClient()) {     //  技师端  收到工单的推送消息时
                            EventBus.getDefault().postSticky(C.EventKey.UPDATE_WORKER_NUMBER);
                        }
                        if (message.getStringAttribute("remindType", "").equals(RemindInfo.REMIND_TYPE_ORDER_REFUND) && App.getInstance().isClient()) {     //  收到 商品退款通知时  刷新账户余额
                   ...
                   /* if(!isChatPageShow&&message.getChatType() == ChatType.Chat){
                        getChatUserInfoById(message.getFrom());
                    }*/
                }
            }
    ...
    private void getChatUserInfoById(final String userNo) {
        // if (DbHelper.getInstance().getUserByNum(userNo) == null) {
        if (callHashMap.containsKey(userNo)) {
            return;
        }
        Call<ResultBase<User>> call = App.getInstance().getHxApiService().getUserByNum(App.getInstance().getCurrentUserNum(), userNo);
        callHashMap.put(userNo, call);
        call.enqueue(new ResponseCallback<ResultBase<User>>() {
            @Override
            public void onSuccess(Call call, ResultBase<User> result) {
                callHashMap.remove(userNo);
    ...
    public class MyContactListener
        ...
        public void onContactInvited(String username, String reason) {
            ...
            notifyNewInviteMessage(msg);
            EventBus.getDefault().post(new Event<String>(C.EventKey.NEW_FRENDS_INVITATION, ""));
            broadcastManager.sendBroadcast(new Intent(Constant.ACTION_CONTACT_CHANAGED));
        }
        ...
        public void onContactAgreed(String username) {
            ...
            notifyNewInviteMessage(msg);
            EventBus.getDefault().post(new Event<String>(C.EventKey.NEW_FRENDS_ACCEPT, ""));
            broadcastManager.sendBroadcast(new Intent(Constant.ACTION_CONTACT_CHANAGED));
        }
