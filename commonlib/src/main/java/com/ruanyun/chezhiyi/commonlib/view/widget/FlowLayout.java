...
public class FlowLayout extends ViewGroup {
    ...
    private int maxLineUsed = 0;//最大行宽
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < this.getChildCount(); i++) {
            ...
            if (lineUsed + spaceWidth > widthSize) {
                ...
                int previousLineUsed = lineUsed; //保存上一行的长度
                lineY += lineHeight + lineSpacing;
                lineUsed = mPaddingLeft + mPaddingRight;
                lineHeight = 0;
                maxLineUsed = java.lang.Math.max(previousLineUsed, lineUsed);
            }
        ...
        setMeasuredDimension((widthMode == MeasureSpec.EXACTLY) ? widthSize
                : Math.max(lineUsed, maxLineUsed), (heightMode == MeasureSpec.EXACTLY) ? heightSize
...
//        return new MarginLayoutParams(super.generateDefaultLayoutParams());
        return new FlowLayout.LayoutParams(super.generateDefaultLayoutParams());
    }
