...
public class EaseNotifier {
    ...
    protected final static String[] msg_ch = { "收到一条消息", "发来一张图片", "发来一段语音", "发来位置信息", "发来一个视频", "发来一个文件",
                                               "%1个联系人发来%2条消息"
                                             };
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try {
            ...
            if(numIncrease && TextUtils.isEmpty(message.getStringAttribute("remindType",""))){ //不是后台消息计数
            ...
            if (notificationInfoProvider != null) {
                if(!TextUtils.isEmpty(message.getStringAttribute("remindType",""))){//后台推送消息
                    summaryBody= ((EMTextMessageBody)message.getBody()).getMessage();
                } else {
                    // lastest text
