...
//    }
    ...
    static class messageSendCallback implements EMCallBack{

        WeakReference<EaseChatRow> rowWeakReference;
        WeakReference<Activity> activityWeakReference;
        public messageSendCallback(EaseChatRow chatRow,Activity activity){
            rowWeakReference=new WeakReference<EaseChatRow>(chatRow);
            activityWeakReference=new WeakReference<Activity>(activity);
        }
        @Override
        public void onSuccess() {
            EaseChatRow chatRow=rowWeakReference.get();
            if(chatRow!=null)
                chatRow.updateView();
        }
        ...
        @Override
        public void onProgress(final int progress, String status) {
            final EaseChatRow chatRow=rowWeakReference.get();
            Activity activity=activityWeakReference.get();
            if(chatRow!=null&&activity!=null) {
                chatRow.updateView();
                activity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if( chatRow.percentageView!=null)
                            chatRow.percentageView.setText(new StringBuilder(Integer.toString(progress)).append("%").toString());
                    }
                });
            }
        }
    ...
    protected void setMessageSendCallback(){
        if(messageSendCallback == null) {
            messageSendCallback = new messageSendCallback(this, activity);
        }
        message.setMessageStatusCallback(messageSendCallback);
    }
...
//    }
    ...
    static class messageReceiveCallback implements EMCallBack{

        WeakReference<EaseChatRow> rowWeakReference;
        WeakReference<Activity> activityWeakReference;
        public messageReceiveCallback(EaseChatRow chatRow,Activity activity){
            rowWeakReference=new WeakReference<EaseChatRow>(chatRow);
            activityWeakReference=new WeakReference<Activity>(activity);
        }
        @Override
    ...
    protected void setMessageReceiveCallback(){
        if(messageReceiveCallback == null) {
            messageReceiveCallback = new messageReceiveCallback(this, activity);
        }
        message.setMessageStatusCallback(messageReceiveCallback);
    }
