...
public class EaseChatRowVoicePlayClickListener implements View.OnClickListener {
	...
	public void stopPlayVoice() {
		...
		if (message.direct() == EMMessage.Direct.RECEIVE) {
			voiceIconView.setImageResource(R.drawable.btn_voice_green_3);
		} else {
			voiceIconView.setImageResource(R.drawable.btn_voice_white_3);
		}
		// stop play voice
		if (mediaPlayer != null) {
			mediaPlayer.stop();
	...
	public void onClick(View v) {
		...
		if (message.direct() == EMMessage.Direct.SEND) {
			...
			if(TextUtils.isEmpty(voiceBody.getLocalUrl())){
				new AsyncDownladTask().execute();
			}else if(new File(FileDownloader.getCacheVoicePath(message.getMsgId())).exists()){
				playVoice(FileDownloader.getCacheVoicePath(message.getMsgId()));
			}else {
	...
	class AsyncDownladTask extends AsyncTask<Void, Void, String>{

		@Override
		protected String doInBackground(Void... params) {
			return FileDownloader.downLoad(voiceBody.getRemoteUrl(),message.getMsgId());
		}
